...
const Profile = () => {
  const [profileData, setProfileData] = useState({
    name: localStorage.getItem('userName') || 'Emma Smith',
    email: localStorage.getItem('userEmail') || 'emma.smith@gmail.com',
    phone: localStorage.getItem('userPhone') || '+1 (123) 456-7890',
    dob: localStorage.getItem('userDob') || '15 July 1990',
    joinedDate: localStorage.getItem('userJoinedDate') || 'March 12, 2020',
    role: (localStorage.getItem('userRole') || 'Administrator').charAt(0).toUpperCase() + (localStorage.getItem('userRole') || 'Administrator').slice(1),
    title: localStorage.getItem('userTitle') || 'Front-End Developer',
    location: localStorage.getItem('userLocation') || 'San Francisco',
    bio: localStorage.getItem('userBio') || 'I manage user roles, oversee platform settings, and ensure everything runs smoothly and securely. With a focus on performance, privacy, and efficiency, I streamline daily operations so your team can focus on what matters most.',
    avatar: localStorage.getItem('userAvatar') || '/profile_avatar.png',
    password: '••••••••'
  });
  ...
  return (
    ...
    >
      {/* Dynamic Header Card */}
      <div className="glass" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '32px', borderRadius: '16px', background: '#ffffff', border: '1px solid var(--border)', boxShadow: '0 4px 20px -2px rgba(0, 0, 0, 0.05)' }}>
        <div style={{ display: 'flex', gap: '28px', alignItems: 'center' }}>
          <div style={{ position: 'relative' }}>
            <img 
              src={profileData.avatar} 
              alt={profileData.name} 
              style={{ width: '92px', height: '92px', borderRadius: '50%', objectFit: 'cover', border: '3px solid #ffffff', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }}
              onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=256&q=80'; }}
            />
            <button 
              title="Change Profile Picture"
              style={{ position: 'absolute', top: '-4px', right: '-4px', background: '#2563eb', border: '2px solid #ffffff', borderRadius: '50%', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#ffffff', boxShadow: '0 2px 5px rgba(0,0,0,0.2)' }}
            >
              <Camera size={14} />
            </button>
          </div>
          ...
          <div>
            <h2 style={{ fontSize: '24px', fontWeight: 700, color: '#0f172a', margin: 0 }}>{profileData.name}</h2>
            <p style={{ fontSize: '14px', color: '#64748b', margin: '6px 0 14px 0', fontWeight: 500 }}>{profileData.title}</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <span style={{ display: 'inline-block', padding: '4px 12px', background: '#f97316', color: '#ffffff', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>
                Designer
              </span>
              <span style={{ display: 'inline-block', padding: '4px 12px', background: '#22c55e', color: '#ffffff', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>
                Active
              </span>
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '28px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <div className="glass" style={{ padding: '24px', borderRadius: '16px', background: '#ffffff', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', paddingBottom: '14px', marginBottom: '20px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#0f172a', margin: 0 }}>Basic Information</h3>
              <button 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <div style={{ fontSize: '14px', color: '#1e293b', fontWeight: 600, marginTop: '4px' }}>{profileData.name}</div>
              </div>
              <div>
                <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Email</span>
                <div style={{ fontSize: '14px', color: '#1e293b', fontWeight: 600, marginTop: '4px', wordBreak: 'break-all' }}>{profileData.email}</div>
              </div>
              <div>
                <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Phone</span>
                <div style={{ fontSize: '14px', color: '#1e293b', fontWeight: 600, marginTop: '4px' }}>{profileData.phone}</div>
              </div>
              <div>
                <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Date of Birth</span>
                <div style={{ fontSize: '14px', color: '#1e293b', fontWeight: 600, marginTop: '4px' }}>{profileData.dob}</div>
              </div>
              <div>
                <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Joined Date</span>
                <div style={{ fontSize: '14px', color: '#1e293b', fontWeight: 600, marginTop: '4px' }}>{profileData.joinedDate}</div>
              </div>
            </div>
          </div>
          ...
          <div className="glass" style={{ padding: '24px', borderRadius: '16px', background: '#ffffff', border: '1px solid var(--border)' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#0f172a', margin: '0 0 18px 0', borderBottom: '1px solid #f1f5f9', paddingBottom: '14px' }}>Social Media Links</h3>
          ...
          {/* Expertise Card */}
          <div className="glass" style={{ padding: '24px', borderRadius: '16px', background: '#ffffff', border: '1px solid var(--border)' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#0f172a', margin: '0 0 18px 0', borderBottom: '1px solid #f1f5f9', paddingBottom: '14px' }}>Expertise</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          <div className="glass" style={{ padding: '32px', borderRadius: '16px', background: '#ffffff', border: '1px solid var(--border)' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 700, color: '#0f172a', margin: '0 0 24px 0', borderBottom: '1px solid #f1f5f9', paddingBottom: '16px' }}>Account Settings</h3>
            
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label htmlFor="form-name" style={{ fontSize: '13px', color: '#64748b', fontWeight: 600 }}>Full Name</label>
                  <input 
                    type="text" 
                    id="form-name"
                    name="name" 
                    ...
                    style={{ padding: '12px 16px', background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: '8px', color: '#1e293b', outline: 'none', fontSize: '14px' }}
                  />
                </div>
                
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label htmlFor="form-email" style={{ fontSize: '13px', color: '#64748b', fontWeight: 600 }}>Email</label>
                  <input 
                    type="email" 
                    id="form-email"
                    name="email" 
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label htmlFor="form-phone" style={{ fontSize: '13px', color: '#64748b', fontWeight: 600 }}>Phone</label>
                  <input 
                    type="text" 
                    id="form-phone"
                    name="phone" 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label htmlFor="form-role" style={{ fontSize: '13px', color: '#64748b', fontWeight: 600 }}>Role</label>
                  <input 
                    type="text" 
                    id="form-role"
                    name="role" 
                    value={editForm.role} 
                    onChange={handleChange}
                    style={{ padding: '12px 16px', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', color: '#64748b', outline: 'none', fontSize: '14px', cursor: 'not-allowed' }}
                    disabled
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label htmlFor="form-location" style={{ fontSize: '13px', color: '#64748b', fontWeight: 600 }}>Location</label>
                  <input 
                    type="text" 
                    id="form-location"
                    name="location" 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label htmlFor="form-password" style={{ fontSize: '13px', color: '#64748b', fontWeight: 600 }}>New Password</label>
                  <input 
                    type="password" 
                    id="form-password"
                    name="password" 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label htmlFor="form-bio" style={{ fontSize: '13px', color: '#64748b', fontWeight: 600 }}>Bio</label>
                <textarea 
                  id="form-bio"
                  name="bio" 
                  value={editForm.bio} 
                  onChange={handleChange}
                  rows="4"
                  style={{ padding: '14px 16px', background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: '8px', color: '#1e293b', outline: 'none', fontSize: '14px', resize: 'vertical', lineHeight: '1.6' }}
                />
              </div>
          ...
          {/* Danger Zone Card */}
          <div style={{ padding: '32px', borderRadius: '16px', background: '#fef2f2', border: '1px solid #fee2e2' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 700, color: '#dc2626', margin: 0 }}>Danger Zone</h3>
            <p style={{ fontSize: '13px', color: '#b91c1c', margin: '4px 0 24px 0', fontWeight: 500 }}>Critical actions that affect your account.</p>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '20px', borderBottom: '1px solid #fca5a5' }}>
                <div style={{ paddingRight: '20px' }}>
                  <h4 style={{ fontSize: '15px', fontWeight: 600, color: '#dc2626', margin: 0 }}>Delete Account</h4>
                  <p style={{ fontSize: '12px', color: '#991b1b', margin: '6px 0 0 0', lineHeight: 1.5 }}>This action is <strong>permanent</strong> and cannot be undone. Please make sure you really want to delete your account.</p>
                </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ paddingRight: '20px' }}>
                  <h4 style={{ fontSize: '15px', fontWeight: 600, color: '#4f46e5', margin: 0 }}>Export Your Data</h4>
                  <p style={{ fontSize: '12px', color: '#3730a3', margin: '6px 0 0 0', lineHeight: 1.5 }}>Backup your data in case you decide to delete your account later.</p>
                </div>
                <button className="btn-secondary" style={{ border: '1px solid #4f46e5', color: '#4f46e5', background: 'transparent', padding: '10px 20px', borderRadius: '8px', fontSize: '14px', fontWeight: 600, flexShrink: 0 }}>
                  Export Data
                </button>
              </div>
